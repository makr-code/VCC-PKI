...
.DESCRIPTION
    Stops the running VCC PKI Server gracefully.
...
Write-ColorOutput "    VCC PKI Server - Stop Script" "Cyan"
Write-ColorOutput "═══════════════════════════════════════════════════════" "Cyan"
Write-Host ""

# Check if PID file exists
if (-not (Test-Path $PID_FILE)) {
    Write-Warning-Msg "PKI Server is not running (no PID file found)"
    exit 0
}
...
# Stop process
Write-Info "Stopping PKI Server (PID: $processId)..."
...
try {
    if ($Force) {
        ...
        Write-Success "Server stopped gracefully"
    }
    ...
    Write-Error-Msg "Failed to stop server: $_"
    exit 1
}

Write-Host ""
Write-Success "PKI Server stopped successfully!"
Write-Host ""
